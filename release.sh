...
for i in $SYS; do
    ...
    env GOOS=$OS GOARCH=$ARCH go build -v -ldflags "$COMMITFLAGS" github.com/vulcanize/lnd
    env GOOS=$OS GOARCH=$ARCH go build -v -ldflags "$COMMITFLAGS" github.com/vulcanize/lnd/cmd/lncli
    cd ..
    if [[ $OS = "windows" ]]; then
